      ...
      {loading ? (
        ...
        Object.keys(groupedTodos).map((date) => (
          <div key={date} className="mb-6">
            <h2 className="text-lg text-primary font-normal mb-2">{date}</h2>
            <div className="space-y-4">
              {groupedTodos[date].map((todo: any) => (
                <div
                  key={todo.id}
                  className="p-4  shadow-md border-gray-300 rounded-md flex justify-between items-center"
                >
                  <div>
                    <h3 className="text-lg font-normal">{todo.title}</h3>
                    <p className="mt-2">
                      <span
